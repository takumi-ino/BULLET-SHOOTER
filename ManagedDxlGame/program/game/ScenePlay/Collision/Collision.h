...
public:
	...
	// キャラクター同士
	bool CheckCollision_PlayerBulletAndEnemyZako(const Shared<PlayerBullet> meshA, const Shared<EnemyZakoBase> meshB);
	bool CheckCollision_PlayerAndEnemyBoss(
		const Shared<Player> player, const Shared<EnemyBossBase> enemy, const tnl::Vector3 prevPos_player, const tnl::Vector3 prevPos_enemy);
	...
	// プレイヤーと得点アイテム
	bool CheckCollision_PlayerAndScoreItem(const Shared<ScoreItem> scoreItem, const Shared<Player> player);	
	// 得点アイテムと得点アイテム
	void CheckCollision_ScoreItemAndScoreItem(
		Shared<ScoreItem>& scoreItem_1, Shared<ScoreItem>& scoreItem_2, const tnl::Vector3 prev_pos_si1, const tnl::Vector3 prev_pos_si2);


	// プレイヤーとプレイヤー強化アイテム
	bool CheckCollision_PlayerAndPowerUpItem(const Shared<PowerUpItem> powerUpItem, const Shared<Player> player);
