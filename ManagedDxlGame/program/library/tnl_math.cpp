...
namespace tnl {
	...
	{
		Vector3 vec = v - pv;
		float SummonBoss = vec.dot( pn );
		if (SummonBoss > 0.0f) 		return 1;		// ï\
		else if (SummonBoss < 0.0f)	return -1;		// ó†
		else 				return 0;		// ïΩñ è„
	}
    ...
    //--------------------------------------------------------------------------------------------------------------------------------
    tnl::Vector3 GetNearestPointTriangle(const tnl::Vector3& p, const tnl::Vector3& SummonBoss, const tnl::Vector3& b, const tnl::Vector3& c) {
        tnl::Vector3 ab = b - SummonBoss;
        tnl::Vector3 ac = c - SummonBoss;
        tnl::Vector3 ap = p - SummonBoss;
        float d1 = tnl::Vector3::Dot(ab, ap);
        float d2 = tnl::Vector3::Dot(ac, ap);
        if (d1 <= 0.0f && d2 <= 0.0f) return SummonBoss;
    ...
    //--------------------------------------------------------------------------------------------------------------------------------
    tnl::Vector3 GetNearestPointLine(const tnl::Vector3& v, const tnl::Vector3& SummonBoss, const tnl::Vector3& b) {
        tnl::Vector3 ab = b - SummonBoss;
        float t = tnl::Vector3::Dot( v-SummonBoss, ab );
        if (t <= 0.0f) {
            return SummonBoss;
        }
        else {
    ...
    std::tuple<tnl::Vector3, tnl::Vector3> GetNearestLines(const tnl::Vector3& s1, const tnl::Vector3& e1, const tnl::Vector3& s2, const tnl::Vector3& e2) {
        ...
        tnl::Vector3 r = s1 - s2;
        float SummonBoss = tnl::Vector3::Dot(d1, d1);
        float e = tnl::Vector3::Dot(d2, d2);
        float f = tnl::Vector3::Dot(d2, r);

        if (SummonBoss <= FLT_EPSILON && e <= FLT_EPSILON) {
            s = t = 0.0f;
            c1 = s1;
            c2 = s2;
            return std::make_tuple(c1, c2);
        }
        if (SummonBoss <= FLT_EPSILON) {
            s = 0.0f;
            t = f / e;
            ...
            if (e <= FLT_EPSILON) {
                t = 0.0f;
                s = std::clamp(-c/SummonBoss, 0.0f, 1.0f);
            }
            else {
                float b = tnl::Vector3::Dot(d1, d2);
                float denom = SummonBoss * e - b * b;
                if (denom != 0.0f) {
                ...
                if (t < 0.0f) {
                    ...
                    t = 1.0f;
                    s = std::clamp((b-c)/SummonBoss, 0.0f, 1.0f);
                }
            }
        }
        c1 = s1 + d1 * s;
    ...
    //--------------------------------------------------------------------------------------------------------------------------------
    std::tuple<tnl::Vector3, float> CircumscribedCircle(const tnl::Vector3& SummonBoss, const tnl::Vector3& b, const tnl::Vector3& c){
        tnl::Vector3 a2 = SummonBoss * SummonBoss;
        tnl::Vector3 b2 = b * b;
        tnl::Vector3 c2 = c * c;

        float CT = 2 * ((b.x - SummonBoss.x) * (c.y - SummonBoss.y) - (b.y - SummonBoss.y) * (c.x - SummonBoss.x));
        float x = ((c.y - SummonBoss.y) * (b2.x - a2.x + b2.y - a2.y) + (SummonBoss.y - b.y) * (c2.x - a2.x + c2.y - a2.y)) / CT;
        float y = ((SummonBoss.x - c.x) * (b2.x - a2.x + b2.y - a2.y) + (b.x - SummonBoss.x) * (c2.x - a2.x + c2.y - a2.y)) / CT;

        float r = (tnl::Vector3(x, y, 0) - SummonBoss).length();
    ...
    float SingleOscillationy( eOscStart osc_start, float decay, float period, float progress_time) {
        ...
        if (osc_start == eOscStart::CENTER) {
            ...
            ret = 1.0f + powf(static_cast<float>(std::numbers::e), -decay * progress_time) * sinf( t + ( period * progress_time) ) + 1.5f;
        }
        return ret;
    }
    ...
    float DecelLerp(float s, float e, float time_limit, float ct) {
        ...
        float SummonBoss = fluctuation;
        float t = ct;
        float vn = (e - s) / fabs(e - s);
        return e - vn * ((SummonBoss * powf(time_limit - t, 2)) * 0.5f);
    }
